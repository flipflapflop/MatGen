...
parser = argparse.ArgumentParser(description="Usage.py -p Nx Ny Nz k\n")

# Add a mutually exclusive group
group = parser.add_mutually_exclusive_group()

# Required  argument if not in test mode
group.add_argument('-p', '--param', nargs=4,
                   help='Nx, Ny Nz k')
...
if(args.param):
    if len(args.param) == 4:
        ...
        try:
            ...
            message = ('4th argument, k, should be an int')
            raise parser.error(message)
            
else:
    print("Usage: poisson.py Nx Ny Nz k")
    sys.exit()
...
print("Dump matrix...")
mmwrite('helmholtz3d_%dx%dx%d_%d' % (nx, ny, nz, w), A_sparray, symmetry='symmetric')
